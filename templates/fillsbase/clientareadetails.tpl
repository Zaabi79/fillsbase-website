...
<form method="post" action="?action=details" role="form" class="bg-seccolorstyle bg-white noshadow mt-50 p-50 br-12">
    ...
    <div class="row">
        <div class="col-sm-6">
            ...
            {if $emailPreferencesEnabled}
                <div class="col-sm-12">
                    <div class="form-group">
                        <label class="control-label mergecolor">{$LANG.clientareacontactsemails}</label>
                        <div style="margin-top:8px;">
                            {foreach $emailPreferences as $emailType => $value}
                                {assign "langKey" "emailPreferences."|cat:$emailType}
                                {assign "labelText" {lang key=$langKey}}
                                {if !$labelText}{assign "labelText" $emailType|replace:'_':' '|ucfirst}{/if}
    ...
    {if $showMarketingEmailOptIn}
        <div class="form-group" style="margin-top:28px;padding-top:20px;border-top:1px solid #eee;">
            <label class="control-label mergecolor" style="font-size:15px;font-weight:700;margin-bottom:6px;display:block;">{lang key='emailMarketing.joinOurMailingList'}</label>
            <p style="font-size:13px;color:#888;margin-bottom:12px;">{$marketingEmailOptInMessage}</p>
            <label style="display:flex;align-items:center;gap:8px;font-weight:normal;cursor:pointer;">
                <input type="checkbox" name="marketingoptin" value="1"{if $marketingEmailOptIn} checked{/if} />
                {lang key='emailMarketing.optIn'}
            </label>
        </div>
    {/if}

    <div class="form-group" style="margin-top:28px;padding-top:20px;border-top:1px solid #eee;">
        <button type="submit" name="save" class="btn btn-default-yellow-fill" style="margin-right:10px;">{$LANG.clientareasavechanges}</button>

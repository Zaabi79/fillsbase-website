...
//----------------------------------------------------/
// Dynamic Brand Name Configuration
//----------------------------------------------------/
function dynamicBrand() {
    const brandConfig = {
        name: 'Fillsbase',
        domain: 'fillsbase.com'
    };

    document.querySelectorAll('.dynamic-brand-name').forEach(el => {
        const type = el.getAttribute('data-brand');
        if (type === 'domain') {
            el.textContent = brandConfig.domain;
        } else if (type === 'name') {
            el.textContent = brandConfig.name;
        } else {
            // Fallback heuristics for existing spans without data-attribute
            if (el.textContent.toLowerCase().includes('.africa') || el.textContent.toLowerCase().includes('.com')) {
                el.textContent = brandConfig.domain;
            } else {
                el.textContent = brandConfig.name;
            }
...
function settings() {
    ...
    if ($settings.length && $.trim($settings.html()).length === 0) {
...
function headerfooter() {
    var $header = $("#header");
    if ($header.length && $.trim($header.html()).length === 0) {
        $header.load("header.php", function(res, status, xhr) {
    ...
    var $footer = $("#footer");
    if ($footer.length && $.trim($footer.html()).length === 0) {
        $footer.load("footer.php", function(res, status, xhr) {
            $('#drop-lng [data-lng="' + localStorage.getItem('lng') + '"]')
            .addClass('xpto active');
            $('#drop-lng label')
            .click(function(e) {
